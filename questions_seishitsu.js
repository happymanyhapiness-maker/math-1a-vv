/* =================================================================
   図形の性質(数学A) 拡張版 — 21問
   既存の questions_keiryo.js (図形と計量) と同じフォーマット。
   単元選択UIから "seishitsu" として読み込まれることを想定。
================================================================= */

const ROUTE_CHOICES_SEISHITSU = [
  "重心の性質",
  "二等辺三角形の性質",
  "三平方",
  "外心の性質",
  "垂心の性質",
  "チェバの定理",
  "メネラウスの定理",
  "円周角の定理",
  "接弦角の定理",
  "内接四角形の条件",
  "方べきの定理",
  "2円の位置関係",
  "共通接線の長さ",
  "共通点の抽出",
  "接線の長さ(1点から2本)",
  "角の二等分線の性質"
];

const questions_seishitsu = [

/* =========================
第1問(重心・外心・垂心)
========================= */
{
id: "g1-1",
stage: "第1問",
num: 1,
time: 20,
score: 5,
weakness: "計算精度",
route: ["重心の性質"],
q: "三角形ABCの重心をGとし、頂点Aと辺BCの中点Dを結ぶ線分(中線)をADとする。AG:GDは？",
a: ["2:1", "1:2", "3:1", "1:1"],
correct: 0,
tags: ["correct", "ratio_reverse", "formula_mismatch", "concept_gap"],
explain: {
aim: "重心が中線を2:1に内分するという性質を、比の向きを間違えず使えるかを測る問題。",
why: "重心は三角形の3本の中線の交点であり、各中線を頂点側から2:1に内分する。\n\nしたがって中線ADにおいても、AG:GD=2:1となる。",
mistake: "比を逆にして1:2としてしまうことがある。頂点に近い方が長い(2の方)ことを覚えておく。",
tip: "重心の比は『頂点側が2、対辺側が1』と覚える。"
}
},
{
id: "g1-2",
stage: "第1問",
num: 2,
time: 25,
score: 5,
weakness: "計算精度",
route: ["重心の性質"],
q: "三角形ABCの重心をGとし、頂点Aと辺BCの中点Dを結ぶ中線ADの長さが12であるとき、AGの長さは？",
a: ["8", "6", "4", "9"],
correct: 0,
tags: ["correct", "calc_error", "calc_error", "concept_gap"],
explain: {
aim: "頂点から重心までの長さが中線の$\\dfrac{2}{3}$であるという関係を正しく使えるかを測る問題。",
why: "重心ではAG:GD=2:1となるので、AG:AD=2:(2+1)=2:3。\n\nつまりAGはAD全体の$\\dfrac{2}{3}$にあたる。\n\nしたがって$\\mathrm{AG}=12\\times\\dfrac{2}{3}=8$。",
mistake: "$\\dfrac{2}{3}$ではなく$\\dfrac{1}{2}$や$\\dfrac{1}{3}$をかけてしまうことがある。",
tip: "頂点から重心までの長さは『中線×$\\dfrac{2}{3}$』。"
}
},
{
id: "g1-3",
stage: "第1問",
num: 3,
time: 40,
score: 5,
weakness: "方針切替",
route: ["二等辺三角形の性質", "三平方"],
q: "二等辺三角形ABCにおいて、AB=AC=13、BC=10である。辺BCの中点をMとするとき、線分AMの長さは？",
a: ["12", "11", "10", "√69"],
correct: 0,
tags: ["correct", "calc_error", "concept_gap", "diagram_reading"],
explain: {
aim: "二等辺三角形の対称性から垂線の位置を見抜き、三平方の定理につなげられるかを測る問題。",
why: "二等辺三角形では、底辺の中点と頂角の頂点を結ぶ線分は、底辺に垂直になる(対称性より)。\n\nしたがってAMは底辺BCに垂直な高さでもあり、直角三角形ABMで三平方の定理が使える。\n\n$\\mathrm{BM}=\\dfrac{\\mathrm{BC}}{2}=5$なので、$\\mathrm{AM}^{2}=\\mathrm{AB}^{2}-\\mathrm{BM}^{2}=13^{2}-5^{2}=169-25=144$。\n\nよって$\\mathrm{AM}=\\sqrt{144}=12$。",
mistake: "AMが垂線になることに気づかず、計算を進められなくなることが多い。",
tip: "二等辺三角形が出たら『底辺の中点=垂線の足』を真っ先に疑う。"
}
},
{
id: "g1-4",
stage: "第1問",
num: 4,
time: 35,
score: 5,
weakness: "方針切替",
route: ["外心の性質"],
q: "∠A=90°の直角三角形ABCにおいて、外心Oと頂点Bとの距離(外接円の半径)が7であるとき、辺BCの長さは？",
a: ["14", "7", "10.5", "21"],
correct: 0,
tags: ["correct", "concept_gap", "calc_error", "concept_gap"],
explain: {
aim: "直角三角形の外心が斜辺の中点になるという性質を正しく使えるかを測る問題。",
why: "直角三角形では、斜辺の中点が外心になる。\n\nこれは、斜辺を直径とする円がその直角三角形の外接円になるという性質による。\n\nしたがって外接円の半径は斜辺の半分であり、斜辺BC=2×7=14。",
mistake: "外心の位置を三角形の内部だと思い込み、図形的に処理できなくなることがある。",
tip: "直角三角形が出たら『外心=斜辺の中点』を真っ先に思い出す。"
}
},
{
id: "g1-5",
stage: "第1問",
num: 5,
time: 20,
score: 5,
weakness: "時間判断",
route: ["垂心の性質"],
q: "∠A=90°の直角三角形ABCの垂心Hの位置として正しいものは？",
a: ["頂点Aと一致する", "辺BCの中点である", "三角形の内部にある(頂点とは異なる)", "外心Oと一致する"],
correct: 0,
tags: ["correct", "concept_gap", "concept_gap", "concept_gap"],
explain: {
aim: "垂心の位置が三角形の形によって変わる(直角三角形では頂点と一致する)ことを理解しているかを測る問題。",
why: "垂心は各頂点から対辺に下ろした垂線の交点。\n\n直角三角形では、∠Aをつくる2辺AB,ACがそれぞれ対辺AC,ABに垂直になっているため、頂点Aそのものがすでに2本の垂線の交点になっている。\n\nしたがって垂心Hは頂点Aと一致する。",
mistake: "垂心は必ず三角形の内部にあると思い込んでしまうことがある。",
tip: "直角三角形・鈍角三角形では、垂心の位置が特殊になることを覚えておく。"
}
},

/* =========================
第1問追加(角の二等分線)
========================= */
{
id: "g5-4",
stage: "第1問",
num: 6,
time: 40,
score: 5,
weakness: "計算精度",
route: ["角の二等分線の性質"],
q: "△ABCにおいて、AB=9、AC=6、BC=10とする。∠Aの二等分線と辺BCの交点をDとするとき、線分BDの長さを求めよ。",
a: ["6", "4", "5", "9"],
correct: 0,
tags: ["correct", "ratio_reverse", "concept_gap", "concept_gap"],
explain: {
aim: "角の二等分線の定理(BD:DC=AB:AC)を、どちらの比をどちら側に対応させるか間違えずに使えるかを測る問題。",
why: "角の二等分線の定理より、∠Aの二等分線が対辺BCを分ける比は、隣り合う2辺の比に等しい。\n\nBD:DC=AB:AC=9:6=3:2。\n\nBC=10をこの比で分けるので、$\\mathrm{BD}=10\\times\\dfrac{3}{3+2}=10\\times\\dfrac{3}{5}=6$。",
mistake: "比の対応を逆にして、BD:DC=AC:ABとして計算すると、$\\mathrm{BD}=10\\times\\dfrac{2}{5}=4$(これは本当はDCの値)を答えてしまう。『B側の長さはB側の頂点に近い辺Aの比』という対応を取り違えやすい。",
tip: "角の二等分線の定理は『Dに近い方の頂点」と「遠い方の頂点」で覚えず、『BD:DC=AB:AC(二等分される角の両側の辺の比)』とセットで丸暗記する。迷ったら、AB=ACの二等辺三角形ならDが中点になる(BD:DC=1:1=AB:AC)ことで検算する。"
}
},

/* =========================
第2問(チェバ・メネラウス)
========================= */
{
id: "g2-1",
stage: "第2問",
num: 1,
time: 30,
score: 5,
weakness: "計算精度",
route: ["チェバの定理"],
q: "三角形ABCの辺BC,CA,AB上にそれぞれ点D,E,Fがあり、線分AD,BE,CFが1点で交わっている。BD:DC=2:3、CE:EA=4:1のとき、AF:FBは？",
a: ["3:8", "8:3", "3:4", "4:3"],
correct: 0,
tags: ["correct", "ratio_reverse", "calc_error", "ratio_reverse"],
explain: {
aim: "チェバの定理を、頂点を一周する向きで正しく比を掛けられるかを測る問題。",
why: "チェバの定理(三角形の3本の線分が1点で交わるとき、頂点を一周する向きで比を掛けると1になる)より、$\\dfrac{\\mathrm{BD}}{\\mathrm{DC}}\\times\\dfrac{\\mathrm{CE}}{\\mathrm{EA}}\\times\\dfrac{\\mathrm{AF}}{\\mathrm{FB}}=1$が成り立つ。\n\n$\\dfrac{\\mathrm{BD}}{\\mathrm{DC}}=\\dfrac{2}{3}$、$\\dfrac{\\mathrm{CE}}{\\mathrm{EA}}=\\dfrac{4}{1}$を代入すると、$\\dfrac{2}{3}\\times4\\times\\dfrac{\\mathrm{AF}}{\\mathrm{FB}}=1$。\n\n整理すると$\\dfrac{8}{3}\\times\\dfrac{\\mathrm{AF}}{\\mathrm{FB}}=1$なので、$\\dfrac{\\mathrm{AF}}{\\mathrm{FB}}=\\dfrac{3}{8}$。\n\nしたがってAF:FB=3:8。",
mistake: "3つの比をかける順番を間違えたり、逆数にしてしまうミスが多い。",
tip: "チェバは『B→D→C, C→E→A, A→F→B』と頂点を一周する順で比をかける。"
}
},
{
id: "g2-2",
stage: "第2問",
num: 2,
time: 20,
score: 5,
weakness: "時間判断",
route: ["チェバの定理"],
q: "三角形ABCの内部に点Pがある。3直線AP,BP,CPがそれぞれ対辺BC,CA,ABと交わるとき、3つの比の積が1になるという定理は？",
a: ["チェバの定理", "メネラウスの定理", "角の二等分線の定理", "中点連結定理"],
correct: 0,
tags: ["correct", "concept_gap", "formula_mismatch", "formula_mismatch"],
explain: {
aim: "チェバの定理とメネラウスの定理を、状況(内部の点か、1本の直線か)で見分けられるかを測る問題。",
why: "三角形の内部の1点から3頂点を通る直線が対辺と交わるときの比の関係を表すのはチェバの定理。\n\nメネラウスの定理は、三角形の頂点を通らない1本の直線(辺またはその延長と交わる)についての定理であり、状況が異なる。",
mistake: "『3つの比の積が1』という結論だけで覚えていると、チェバとメネラウスを混同しやすい。",
tip: "『内部の1点から3頂点』→チェバ、『1本の直線が3辺(の延長)を横切る』→メネラウス、と状況で見分ける。"
}
},
{
id: "g2-3",
stage: "第2問",
num: 3,
time: 55,
score: 5,
weakness: "方針切替",
route: ["メネラウスの定理"],
q: "三角形ABCにおいて、辺ABを2:1に内分する点をR(AR:RB=2:1)、辺CAを1:3に内分する点をQ(CQ:QA=1:3)とする。直線QRと辺BCの延長との交点をPとするとき、BP:PCは？",
a: ["3:2", "2:3", "3:1", "1:3"],
correct: 0,
tags: ["correct", "ratio_reverse", "calc_error", "ratio_reverse"],
explain: {
aim: "メネラウスの定理を、内分点と延長上の点が混在する状況でも正しく適用できるかを測る問題。",
why: "メネラウスの定理(三角形の頂点を通らない1本の直線が3辺(の延長)を横切るとき、頂点を一周する向きで比を掛けると1になる)より、$\\dfrac{\\mathrm{BP}}{\\mathrm{PC}}\\times\\dfrac{\\mathrm{CQ}}{\\mathrm{QA}}\\times\\dfrac{\\mathrm{AR}}{\\mathrm{RB}}=1$が成り立つ。\n\n$\\dfrac{\\mathrm{CQ}}{\\mathrm{QA}}=\\dfrac{1}{3}$、$\\dfrac{\\mathrm{AR}}{\\mathrm{RB}}=\\dfrac{2}{1}=2$を代入すると、$\\dfrac{\\mathrm{BP}}{\\mathrm{PC}}\\times\\dfrac{1}{3}\\times2=1$。\n\n整理すると$\\dfrac{\\mathrm{BP}}{\\mathrm{PC}}\\times\\dfrac{2}{3}=1$なので、$\\dfrac{\\mathrm{BP}}{\\mathrm{PC}}=\\dfrac{3}{2}$。\n\nしたがってBP:PC=3:2。",
mistake: "内分点と延長上の点が混在していると、どの比をどこに当てはめるか混乱しやすい。",
tip: "メネラウスは『B→P→C, C→Q→A, A→R→B』の順で一周するように比をかける。"
}
},
{
id: "g2-4",
stage: "第2問",
num: 4,
time: 60,
score: 5,
weakness: "方針切替",
route: ["メネラウスの定理"],
q: "三角形ABCにおいて、辺ABの延長上(Bの外側)の点R、辺BCの延長上(Cの外側)の点P、辺CAの延長上(Cの外側)の点Qが一直線上にある。AR:RB=3:1、BP:PC=2:1のとき、CQ:QAは？",
a: ["1:6", "6:1", "1:3", "3:1"],
correct: 0,
tags: ["correct", "ratio_reverse", "calc_error", "ratio_reverse"],
explain: {
aim: "メネラウスの定理で、延長上の点の位置関係を正しく踏まえて比を立てられるかを測る問題。",
why: "メネラウスの定理(三角形の頂点を通らない1本の直線が3辺(の延長)を横切るとき、頂点を一周する向きで比を掛けると1になる)より、$\\dfrac{\\mathrm{BP}}{\\mathrm{PC}}\\times\\dfrac{\\mathrm{CQ}}{\\mathrm{QA}}\\times\\dfrac{\\mathrm{AR}}{\\mathrm{RB}}=1$が成り立つ。\n\n$\\dfrac{\\mathrm{BP}}{\\mathrm{PC}}=\\dfrac{2}{1}=2$、$\\dfrac{\\mathrm{AR}}{\\mathrm{RB}}=\\dfrac{3}{1}=3$を代入すると、$2\\times\\dfrac{\\mathrm{CQ}}{\\mathrm{QA}}\\times3=1$。\n\n整理すると$\\dfrac{\\mathrm{CQ}}{\\mathrm{QA}}\\times6=1$なので、$\\dfrac{\\mathrm{CQ}}{\\mathrm{QA}}=\\dfrac{1}{6}$。\n\nしたがってCQ:QA=1:6。",
mistake: "延長上の点がどちら側にあるかを見落とすと、比の大小関係と答えが矛盾してしまうことがある。",
tip: "メネラウスでは、点が辺のどちら側の延長上にあるかを問題文で明確にしておくと、図なしでも判断しやすい。"
}
},
{
id: "g2-5",
stage: "第2問",
num: 5,
time: 25,
score: 5,
weakness: "方針切替",
route: ["チェバの定理"],
q: "三角形ABCの3辺BC,CA,ABの中点をそれぞれD,E,Fとする。中線AD,BE,CFが1点(重心)で交わることを、チェバの定理の左辺の値で確認したい。BD:DC=CE:EA=AF:FB=1:1のとき、チェバの定理の左辺$\\dfrac{\\mathrm{BD}}{\\mathrm{DC}}\\times\\dfrac{\\mathrm{CE}}{\\mathrm{EA}}\\times\\dfrac{\\mathrm{AF}}{\\mathrm{FB}}$の値は？",
a: ["1", "2", "1/2", "0"],
correct: 0,
tags: ["correct", "concept_gap", "calc_error", "concept_gap"],
explain: {
aim: "重心の性質を、チェバの定理の特殊な場合として捉え直せるかを測る問題。",
why: "D,E,Fはすべて中点なので、BD:DC=CE:EA=AF:FB=1:1、つまりそれぞれの比は1。\n\nしたがって$\\dfrac{\\mathrm{BD}}{\\mathrm{DC}}\\times\\dfrac{\\mathrm{CE}}{\\mathrm{EA}}\\times\\dfrac{\\mathrm{AF}}{\\mathrm{FB}}=1\\times1\\times1=1$。\n\nチェバの定理は『この値が1のとき3直線は1点で交わる』という定理なので、これは中線が重心で交わることの確認になっている。",
mistake: "重心の性質(2:1)と中点であること(1:1)を混同してしまうことがある。",
tip: "重心が3本の中線の交点であることは、チェバの定理の特殊な場合(すべての比が1:1)として説明できる。"
}
},
{
id: "g2-6",
stage: "第2問",
num: 6,
time: 75,
score: 5,
weakness: "方針切替",
route: ["チェバの定理", "メネラウスの定理"],
q: "三角形ABCの辺BC,CA,AB上にそれぞれ点D,E,Fがあり、3直線AD,BE,CFが1点Pで交わっている。さらに、直線EFと辺BCの延長との交点をQとする。BD:DC=1:2、CE:EA=1:1のとき、まずチェバの定理を用いてAF:FBを求め、次にメネラウスの定理(三角形ABCと直線F,Q,E)を用いてBQ:QCを求めよ。BQ:QCは？",
a: ["1:2", "2:1", "1:1", "1:4"],
correct: 0,
tags: ["correct", "ratio_reverse", "calc_error", "calc_error"],
explain: {
aim: "チェバの定理で求めた比を、メネラウスの定理の式にそのまま再利用できるかを測る問題。",
why: "まずチェバの定理(三角形の3本の線分が1点で交わるとき、頂点を一周する向きで比を掛けると1になる)$\\dfrac{\\mathrm{BD}}{\\mathrm{DC}}\\times\\dfrac{\\mathrm{CE}}{\\mathrm{EA}}\\times\\dfrac{\\mathrm{AF}}{\\mathrm{FB}}=1$より、$\\dfrac{1}{2}\\times1\\times\\dfrac{\\mathrm{AF}}{\\mathrm{FB}}=1$なので、$\\dfrac{\\mathrm{AF}}{\\mathrm{FB}}=2$、AF:FB=2:1。\n\n次に、三角形ABCと直線F,Q,Eにメネラウスの定理(三角形の頂点を通らない1本の直線が3辺(の延長)を横切るとき、頂点を一周する向きで比を掛けると1になる)を使う。$\\dfrac{\\mathrm{BQ}}{\\mathrm{QC}}\\times\\dfrac{\\mathrm{CE}}{\\mathrm{EA}}\\times\\dfrac{\\mathrm{AF}}{\\mathrm{FB}}=1$。\n\n$\\dfrac{\\mathrm{CE}}{\\mathrm{EA}}=1$、$\\dfrac{\\mathrm{AF}}{\\mathrm{FB}}=2$を代入すると、$\\dfrac{\\mathrm{BQ}}{\\mathrm{QC}}\\times1\\times2=1$なので、$\\dfrac{\\mathrm{BQ}}{\\mathrm{QC}}=\\dfrac{1}{2}$。\n\nしたがってBQ:QC=1:2。",
mistake: "チェバで求めたAF:FBの値を、メネラウスの式にそのまま使えることに気づかず、最初からやり直そうとしてしまう。",
tip: "チェバとメネラウスは別々の定理だが、同じ図の中で『前半の結果を後半の式にそのまま使う』問題が共通テストでは典型。"
}
},

/* =========================
第2問・大問形式(チェバ+メネラウス連続問題)
========================= */
{
id: "g2-7",
stage: "第2問",
num: 7,
time: 50,
score: 5,
weakness: "計算精度",
route: ["チェバの定理"],
group: "g2-chain1",
groupIntro: "三角形ABCにおいて、辺BC上の点D、辺CA上の点Eをとり、線分ADとBEの交点をPとする。直線CPと辺ABの交点をFとする。BD:DC=2:1、CE:EA=1:3とする。",
recap: [],
q: "チェバの定理を用いて、AF:FBを求めよ。",
a: ["3:2", "2:3", "3:1", "1:3"],
correct: 0,
tags: ["correct", "ratio_reverse", "calc_error", "ratio_reverse"],
explain: {
aim: "チェバの定理を使って、比を図と対応させながら正確に立てて計算できるかを測る問題。",
why: "チェバの定理(三角形の3本の線分が1点で交わるとき、頂点を一周する向きで比を掛けると1になる)より、$\\dfrac{\\mathrm{BD}}{\\mathrm{DC}}\\times\\dfrac{\\mathrm{CE}}{\\mathrm{EA}}\\times\\dfrac{\\mathrm{AF}}{\\mathrm{FB}}=1$が成り立つ。\n\n$\\dfrac{\\mathrm{BD}}{\\mathrm{DC}}=2$、$\\dfrac{\\mathrm{CE}}{\\mathrm{EA}}=\\dfrac{1}{3}$を代入すると、$2\\times\\dfrac{1}{3}\\times\\dfrac{\\mathrm{AF}}{\\mathrm{FB}}=1$。\n\n整理すると$\\dfrac{2}{3}\\times\\dfrac{\\mathrm{AF}}{\\mathrm{FB}}=1$なので、$\\dfrac{\\mathrm{AF}}{\\mathrm{FB}}=\\dfrac{3}{2}$。\n\nしたがってAF:FB=3:2。",
mistake: "図を描かずに比だけで処理しようとすると、どの辺の比をかけているか混乱しやすい。",
tip: "図を描かずに比だけで処理すると、どの辺の比を掛けているか混乱しやすい。特に、後で同じ比を複数の式で使い回す問題では、最初に確定した比ほど図に書き込んで確認しておく。"
}
},
{
id: "g2-8",
stage: "第2問",
num: 8,
time: 60,
score: 5,
weakness: "計算精度",
route: ["メネラウスの定理"],
group: "g2-chain1",
groupIntro: "三角形ABCにおいて、辺BC上の点D、辺CA上の点Eをとり、線分ADとBEの交点をPとする。直線CPと辺ABの交点をFとする。BD:DC=2:1、CE:EA=1:3とする。",
recap: ["(1) チェバの定理より、AF:FB = 3:2"],
q: "上の図形で、直線DEと辺ABの延長との交点をQとする。メネラウスの定理を用いて、AQ:QBを求めよ。",
a: ["3:2", "2:3", "1:2", "2:1"],
correct: 0,
tags: ["correct", "ratio_reverse", "calc_error", "ratio_reverse"],
explain: {
aim: "前の設問で求めた比を、似た形の別の定理(メネラウス)の式にそのまま再利用できるかを測る問題。",
why: "三角形ABCと直線D,E,Qにメネラウスの定理(三角形の頂点を通らない1本の直線が3辺(の延長)を横切るとき、頂点を一周する向きで比を掛けると1になる)を使うと、$\\dfrac{\\mathrm{BD}}{\\mathrm{DC}}\\times\\dfrac{\\mathrm{CE}}{\\mathrm{EA}}\\times\\dfrac{\\mathrm{AQ}}{\\mathrm{QB}}=1$が成り立つ。\n\n$\\dfrac{\\mathrm{BD}}{\\mathrm{DC}}=2$、$\\dfrac{\\mathrm{CE}}{\\mathrm{EA}}=\\dfrac{1}{3}$を代入すると、$2\\times\\dfrac{1}{3}\\times\\dfrac{\\mathrm{AQ}}{\\mathrm{QB}}=1$。\n\n整理すると$\\dfrac{2}{3}\\times\\dfrac{\\mathrm{AQ}}{\\mathrm{QB}}=1$なので、$\\dfrac{\\mathrm{AQ}}{\\mathrm{QB}}=\\dfrac{3}{2}$。\n\nしたがってAQ:QB=3:2。",
mistake: "(1)のチェバの定理の式と見た目がほぼ同じなので、AFをAQに、FBをQBに置き換えるだけで済むことに気づかず、ゼロから図を見直してしまうことがある。",
tip: "チェバとメネラウスで同じD,Eを使っているときは、立てる式の左側2つの比(BD/DCとCE/EA)はそのまま使い回せる。"
}
},
{
id: "g2-9",
stage: "第2問",
num: 9,
time: 45,
score: 5,
weakness: "方針切替",
route: ["チェバの定理", "メネラウスの定理"],
group: "g2-chain1",
groupIntro: "三角形ABCにおいて、辺BC上の点D、辺CA上の点Eをとり、線分ADとBEの交点をPとする。直線CPと辺ABの交点をFとする。BD:DC=2:1、CE:EA=1:3とする。",
recap: ["(1) チェバの定理より、AF:FB = 3:2", "(2) メネラウスの定理より、AQ:QB = 3:2"],
q: "(1)で求めたAF:FBと、(2)で求めたAQ:QBは、どちらも3:2であった。この2つの比が一致する理由として正しいものはどれか。",
a: [
"チェバの定理とメネラウスの定理の式が、BD/DCとCE/EAの部分を共通に使っており、残りの比(AF/FBとAQ/QB)がどちらも同じ値になるように決まるから。",
"BD:DCとCE:EAの値がたまたま等しい比になっていたから。",
"点Fと点Qがどちらも直線AB上にあるから。",
"三角形ABCが特別な形(二等辺三角形など)だったから。"
],
correct: 0,
tags: ["correct", "concept_gap", "concept_gap", "concept_gap"],
explain: {
aim: "2つの定理の式の構造的な類似性から、結果が一致する理由を論理的に説明できるかを測る問題。",
why: "チェバの定理(三角形の3本の線分が1点で交わるとき、頂点を一周する向きで比を掛けると1になる): $\\dfrac{\\mathrm{BD}}{\\mathrm{DC}}\\times\\dfrac{\\mathrm{CE}}{\\mathrm{EA}}\\times\\dfrac{\\mathrm{AF}}{\\mathrm{FB}}=1$\n\nメネラウスの定理(三角形の頂点を通らない1本の直線が3辺(の延長)を横切るとき、頂点を一周する向きで比を掛けると1になる): $\\dfrac{\\mathrm{BD}}{\\mathrm{DC}}\\times\\dfrac{\\mathrm{CE}}{\\mathrm{EA}}\\times\\dfrac{\\mathrm{AQ}}{\\mathrm{QB}}=1$\n\nこの2つの式は、$\\dfrac{\\mathrm{BD}}{\\mathrm{DC}}$と$\\dfrac{\\mathrm{CE}}{\\mathrm{EA}}$の部分がまったく同じであり、式全体の値が1になることも同じである。\n\nしたがって、$\\dfrac{\\mathrm{AF}}{\\mathrm{FB}}$と$\\dfrac{\\mathrm{AQ}}{\\mathrm{QB}}$は同じ値にならざるを得ない。これはBD:DCやCE:EAの具体的な数値に関係なく、いつでも成り立つ。",
mistake: "(1)(2)の答えが同じ比になったのを『たまたま』『この問題の数値がたまたま』と思い込み、一般的な構造に気づかないことが多い。",
tip: "2つの定理を同じ図形・同じ点に対して使うとき、式の形が似ていたら『なぜ似ているのか』を一度立ち止まって考えると、計算ミスの発見にもつながる。"
}
},

/* =========================
第3問(円周角・接弦角・内接四角形)
========================= */
{
id: "g3-1",
stage: "第3問",
num: 1,
time: 20,
score: 5,
weakness: "計算精度",
route: ["円周角の定理"],
q: "円Oの周上に3点A,B,Cがある。∠ACB=40°のとき、中心角∠AOB(180°以下のもの)の大きさは？",
a: ["80°", "40°", "20°", "160°"],
correct: 0,
tags: ["correct", "concept_gap", "calc_error", "calc_error"],
explain: {
aim: "円周角と中心角の関係(中心角は円周角の2倍)を正しく使えるかを測る問題。",
why: "円周角の定理より、同じ弧に対する中心角は円周角の2倍になる。\n\nしたがって∠AOB=2×∠ACB=2×40°=80°。",
mistake: "中心角と円周角の関係を逆にして、半分にしてしまうことがある。",
tip: "『中心角は円周角の2倍』、逆に『円周角は中心角の半分』。"
}
},
{
id: "g3-2",
stage: "第3問",
num: 2,
time: 25,
score: 5,
weakness: "計算精度",
route: ["円周角の定理"],
q: "円周上に4点A,B,C,Dがこの順にある。∠ACB=35°のとき、∠ADBの大きさは？",
a: ["35°", "55°", "70°", "17.5°"],
correct: 0,
tags: ["correct", "concept_gap", "calc_error", "calc_error"],
explain: {
aim: "同じ弧に対する円周角が等しいことを、点の位置関係を確認した上で使えるかを測る問題。",
why: "A,B,C,Dがこの順に円周上にあるので、点Cと点Dはいずれも弦ABに対して同じ側にある。\n\n∠ACBと∠ADBは、どちらも同じ弦ABを見込む円周角である。\n\nしたがって円周角の定理より、∠ACB=∠ADB=35°。",
mistake: "同じ弦に対する円周角でも、点の位置関係を確認せずに使うと、等しい角と補角になる角を混同してしまうことがある。",
tip: "同じ弦ABを見込む円周角は、点が弦ABに対して同じ側にあるとき等しくなる。"
}
},
{
id: "g3-3",
stage: "第3問",
num: 3,
time: 40,
score: 5,
weakness: "方針切替",
route: ["接弦角の定理"],
q: "円Oの接線が点Aで円に接している。接線と弦ABの作る角が50°である。弦ABに対して、この接弦角と反対側の弧上に点Cをとるとき、円周角∠ACBの大きさは？",
a: ["50°", "40°", "100°", "25°"],
correct: 0,
tags: ["correct", "concept_gap", "calc_error", "calc_error"],
explain: {
aim: "接弦角の定理を、反対側の弧という向きを含めて正しく使えるかを測る問題。",
why: "接弦角の定理より、接線と弦の作る角は、その弦に対して反対側の弧にある点から見た円周角に等しい。\n\nしたがって∠ACB=50°。",
mistake: "接弦角の定理を円周角の定理(同じ側)と混同し、どちら側の弧を考えるべきか分からなくなることが多い。",
tip: "接弦角は『反対側の弧』の円周角と等しい、と方向をセットで覚える。"
}
},
{
id: "g3-4",
stage: "第3問",
num: 4,
time: 20,
score: 5,
weakness: "計算精度",
route: ["内接四角形の条件"],
q: "円に内接する四角形ABCDにおいて、∠A=70°のとき、∠Cの大きさは？",
a: ["110°", "70°", "290°", "20°"],
correct: 0,
tags: ["correct", "concept_gap", "calc_error", "calc_error"],
explain: {
aim: "円に内接する四角形の対角の和が180°であることを正しく使えるかを測る問題。",
why: "円に内接する四角形では、対角の和は180°になる。\n\nしたがって∠A+∠C=180°なので、∠C=180°-70°=110°。",
mistake: "『対角の和=180°』を『隣り合う角の和=180°』と勘違いすることがある。",
tip: "円に内接する四角形は『向かい合う角を足すと180°』。"
}
},
{
id: "g3-5",
stage: "第3問",
num: 5,
time: 35,
score: 5,
weakness: "方針切替",
route: ["内接四角形の条件"],
q: "四角形ABCDにおいて、∠B=95°、∠D=85°であることが分かっている。この四角形が円に内接するかどうかについて、正しいものは？",
a: ["∠B+∠D=180°なので、円に内接する", "∠A+∠C=180°なので、円に内接する", "対辺が平行なので、円に内接する", "この情報だけでは円に内接するか分からない"],
correct: 0,
tags: ["correct", "concept_gap", "formula_mismatch", "concept_gap"],
explain: {
aim: "対角の和=180°という性質の逆(内接条件の判定)を使えるかを測る問題。",
why: "円に内接する四角形は対角の和が180°になるが、この性質には逆(対角の和が180°ならば円に内接する)も成り立つ。\n\n∠B+∠D=95°+85°=180°なので、四角形ABCDは円に内接すると判断できる。",
mistake: "対角の和の性質を『円に内接する四角形では成り立つ』という一方向だけで覚え、逆を使う発想に気づかないことが多い。",
tip: "『対角の和=180°』は、円に内接する条件としても(逆として)使える。"
}
},
{
id: "g3-6",
stage: "第3問",
num: 6,
time: 70,
score: 5,
weakness: "方針切替",
route: ["円周角の定理"],
q: "円Oに内接する四角形ABCDがこの順にあり、対角線ACとBDの交点をPとする。∠BDC=30°、∠ABD=25°であるとき、∠APBの大きさは？",
a: ["125°", "115°", "105°", "145°"],
correct: 0,
tags: ["correct", "calc_error", "calc_error", "calc_error"],
explain: {
aim: "円周角の定理で作った角を、三角形の内角の和につなげて求められるかを測る問題。",
why: "∠BDCと∠BACは、どちらも弦BCに対して同じ側の弧(A,Dが乗っている側)から見た円周角なので、円周角の定理より∠BAC=∠BDC=30°。\n\n対角線ACとBDの交点をPとすると、三角形APBにおいて、∠PAB=∠BAC=30°、∠PBA=∠ABD=25°。\n\n三角形の内角の和は180°なので、∠APB=180°-30°-25°=125°。",
mistake: "∠BDCがそのまま使える角だと気づかず、∠APBを求める手がかりを見つけられないことが多い。",
tip: "対角線の交点の角を求める問題では、まず円周角の定理で『同じ弧に対する角』を作り、三角形の内角の和につなげる。"
}
},

/* =========================
第3問・大問形式(内接四角形+方べき+接弦角の連続問題)
========================= */
{
id: "g3-7",
stage: "第3問",
num: 7,
time: 25,
score: 5,
weakness: "計算精度",
route: ["内接四角形の条件"],
group: "g3-chain1",
groupIntro: "円Oに内接する四角形ABCDがこの順にあり、対角線ACとBDの交点をPとする。∠BAD=95°、AP=3、PC=8、BP=4とする。",
recap: [],
q: "∠BCDの大きさを求めよ。",
a: ["85°", "95°", "180°", "75°"],
correct: 0,
tags: ["correct", "concept_gap", "concept_gap", "calc_error"],
explain: {
aim: "内接四角形の対角の性質を正しく使えるかを測る問題。",
why: "円に内接する四角形では、対角の和は180°になる。\n\nしたがって∠BAD+∠BCD=180°なので、∠BCD=180°-95°=85°。",
mistake: "対角の和の性質を、隣り合う角の和だと勘違いしてしまうことがある。",
tip: "対角=『向かい合う角』。∠Aの相方は∠C、∠Bの相方は∠D、と組をセットで覚えておくと取り違えない。"
}
},
{
id: "g3-8",
stage: "第3問",
num: 8,
time: 35,
score: 5,
weakness: "計算精度",
route: ["方べきの定理"],
group: "g3-chain1",
groupIntro: "円Oに内接する四角形ABCDがこの順にあり、対角線ACとBDの交点をPとする。∠BAD=95°、AP=3、PC=8、BP=4とする。",
recap: ["(1) 内接四角形の対角の和より、∠BCD = 85°"],
q: "対角線ACとBDの交点Pについて、方べきの定理を用いて、PDの長さを求めよ。",
a: ["6", "8", "24", "4.5"],
correct: 0,
tags: ["correct", "formula_mismatch", "calc_error", "formula_mismatch"],
explain: {
aim: "方べきの定理(交わる弦の線分の積)を正しい組み合わせで使えるかを測る問題。",
why: "円の内部で2つの弦AC,BDが交わるとき、方べきの定理より、AP×PC=BP×PDが成り立つ。\n\nAP=3、PC=8、BP=4を代入すると、3×8=4×PD。\n\n24=4×PDなので、PD=6。",
mistake: "AP×PC=BP×PDの組み合わせを、AP×BP=PC×PDなどと取り違えてしまうことがある。",
tip: "交わる2本の弦では、『交点を挟んで向かい合う2つの線分の積』が等しくなる。"
}
},
{
id: "g3-9",
stage: "第3問",
num: 9,
time: 50,
score: 5,
weakness: "方針切替",
route: ["接弦角の定理", "内接四角形の条件"],
group: "g3-chain1",
groupIntro: "円Oに内接する四角形ABCDがこの順にあり、対角線ACとBDの交点をPとする。∠BAD=95°、AP=3、PC=8、BP=4とする。",
recap: ["(1) 内接四角形の対角の和より、∠BCD = 85°", "(2) 方べきの定理より、PD = 6"],
q: "点Bで円に接する接線をℓとする。接線ℓと弦BDの作る角のうち、弦BDに対してこの接弦角と反対側の弧上に点Cがあるような角の大きさを求めよ。",
a: ["85°", "95°", "180°", "42.5°"],
correct: 0,
tags: ["correct", "concept_gap", "concept_gap", "calc_error"],
explain: {
aim: "接弦角の定理と、前の設問で求めた円周角を結びつけて答えられるかを測る問題。",
why: "接弦角の定理は『弦の端点における接線』と『その弦』の作る角についての定理。ここでは接線ℓが弦BDの端点Bで円に接しているので、定理がそのまま使える。\n\n定理より、接線と弦の作る角は、その弦に対して反対側の弧にある点から見た円周角に等しい。\n\nここでは弦BDに対して反対側の弧上にある点Cから見た円周角、つまり∠BCDに等しい。\n\n∠BCDは(1)で求めた85°なので、この接弦角も85°。",
mistake: "新しく計算しなければならないと思い込み、(1)の結果をそのまま使えることに気づかないことが多い。",
tip: "接弦角の問題では、図の中に『すでに分かっている円周角』がないか、まず探すのが近道。"
}
},

/* =========================
第3問追加(接線の長さ)
========================= */
{
id: "g5-1",
stage: "第3問",
num: 10,
time: 20,
score: 5,
weakness: "計算精度",
route: ["接線の長さ(1点から2本)"],
q: "円Oの外部の点Pから円Oに2本の接線を引き、接点をそれぞれA、Bとする。PA=9のとき、PBの長さは？",
a: ["9", "4.5", "18", "円の半径によって変わる"],
correct: 0,
tags: ["correct", "concept_gap", "concept_gap", "concept_gap"],
explain: {
aim: "円外の1点から引いた2本の接線の長さが等しいという性質を、単独で正しく使えるかを測る問題。",
why: "円外の1点から円に引いた2本の接線の長さは、必ず等しい。\n\nしたがってPB=PA=9。",
mistake: "『2本の接線』という言葉から、PAとPBを合わせて1本分と考えて半分の4.5にしたり、逆に2倍の18にしたりする誤りが多い。この性質は円の半径やPの位置に関係なく常に成り立つ。",
tip: "『同じ点から引いた接線は長さが等しい』は、内接円・傍接円・2円の共通接線など、あらゆる場面で使う最重要の道具。まずこの1点対1点の形で体に入れておく。"
}
},
{
id: "g5-2",
stage: "第3問",
num: 11,
time: 35,
score: 5,
weakness: "計算精度",
route: ["接線の長さ(1点から2本)"],
q: "△ABCの内接円が辺BC、CA、ABに接する点をそれぞれD、E、Fとする。BD=5、CE=3のとき、辺BCの長さを求めよ。",
a: ["8", "10", "2", "15"],
correct: 0,
tags: ["correct", "formula_mismatch", "concept_gap", "concept_gap"],
explain: {
aim: "内接円の接線の長さの等しさを、正しい頂点の組み合わせ(同じ頂点から出る2本)で適用できるかを測る問題。",
why: "円外の1点から引いた2本の接線の長さは等しいので、頂点Cから出る2本の接線CDとCEについて、CD=CE=3。\n\n辺BCはBDとDCに分かれているので、$\\mathrm{BC}=\\mathrm{BD}+\\mathrm{DC}=5+3=8$。",
mistake: "『同じ辺の両端の接線が等しい』と勘違いして、DがBD=DCの中点であるかのように扱い、$\\mathrm{BD}+\\mathrm{BD}=5+5=10$のように計算してしまう(実際の模試でも、BC=8, CA=4, AB=6のはずが4+4, 2+2, 3+3という誤った2倍パターンで再現したミス)。接線の長さが等しいのは『同じ頂点(同じ点)から出る2本』であって、『同じ辺の両端』ではない。",
tip: "接線の長さの等しさは、必ず「どの頂点(点)から出た2本か」を指でなぞって確認してから使う。DはBとCの両方に関わる点なので、BD(B側)とDC(C側)は別の頂点の接線同士で、一致する保証はない。"
}
},
{
id: "g5-3",
stage: "第3問",
num: 12,
time: 60,
score: 6,
weakness: "方針切替",
route: ["接線の長さ(1点から2本)"],
q: "△ABCの内接円が辺BC、CA、ABに接する点をそれぞれD、E、Fとする。BC=8、CA=7、AB=9のとき、線分AFの長さを求めよ。",
a: ["4", "5", "3", "6"],
correct: 0,
tags: ["correct", "ratio_reverse", "concept_gap", "concept_gap"],
explain: {
aim: "3頂点分の接線の長さの等式を連立させ、1頂点だけの長さを逆算できるかを測る問題(内接円の接線長の標準解法)。",
why: "AF=AE=x、BD=BF=y、CD=CE=zとおく。\n\n各辺の長さより、$y+z=\\mathrm{BC}=8$、$z+x=\\mathrm{CA}=7$、$x+y=\\mathrm{AB}=9$。\n\n3式をすべて足すと、$2(x+y+z)=8+7+9=24$より$x+y+z=12$。\n\nここから$\\mathrm{BC}=8$を引くと、$x=12-8=4$。\n\nしたがってAF=x=4。",
mistake: "$x+y+z$(半周長)から引く辺を取り違えて、求めたい頂点Aの対辺BC(=8)ではなく、Aを含む辺CA(=7)やAB(=9)を引いてしまうミスが多い。『求めたい頂点の対辺を引く』という対応が逆になりやすい。",
tip: "『半周長 − 求めたい頂点の対辺 = その頂点からの接線の長さ』と対応を固定して覚える。検算は、出た3つの値(x,y,z)を元の3つの辺の式に戻して、全部一致するか確認する。"
}
},

/* =========================
第4問(2円の位置関係・共通接線)
========================= */
{
id: "g4-1",
stage: "第4問",
num: 1,
time: 25,
score: 5,
weakness: "方針切替",
route: ["2円の位置関係"],
q: "半径がそれぞれ$r,R$($R\\gt r$)の2つの円があり、中心間の距離が$d$であるとする。2円が互いに離れていて共有点を持たない条件はどれか？",
a: ["d>R+r", "d=R+r", "R-r<d<R+r", "d<R-r"],
correct: 0,
tags: ["correct", "concept_gap", "concept_gap", "concept_gap"],
explain: {
aim: "2円の位置関係を、中心間の距離と半径の和・差の大小関係で正しく判定できるかを測る問題。",
why: "2円の位置関係は中心間の距離$d$と2つの半径$R,r$の大小関係で決まる。\n\n2円がまったく重なり合わず、外部に離れている(共有点を持たない)ためには、中心間の距離が2つの半径の和より大きい必要がある。\n\nしたがって条件は$d\\gt R+r$。",
mistake: "$d=R+r$(外接する、1点で接する)と、$d\\gt R+r$(完全に離れている)を混同することがある。",
tip: "位置関係の各パターン($d\\gt R+r$ / $d=R+r$ / $R-r\\lt d\\lt R+r$ / $d=R-r$ / $d\\lt R-r$)は不等号の向きまでセットで覚える。"
}
},
{
id: "g4-2",
stage: "第4問",
num: 2,
time: 30,
score: 5,
weakness: "計算精度",
route: ["2円の位置関係"],
q: "半径5の円と半径3の円があり、中心間の距離が9であるとき、2円の位置関係は？",
a: ["2円は離れていて共有点を持たない", "外接する(1点で接する)", "2点で交わる", "内接する"],
correct: 0,
tags: ["correct", "concept_gap", "concept_gap", "concept_gap"],
explain: {
aim: "2円の位置関係を、具体的な数値でR+r・R-rと比較して判定できるかを測る問題。",
why: "2つの半径の和は$R+r=5+3=8$。\n\n中心間の距離$d=9$は$R+r=8$より大きいので、2円は互いに離れていて共有点を持たない。",
mistake: "$R+r$と$R-r$を取り違え、内接・外接の判定を誤ることがある。",
tip: "まず$R+r$と$R-r$の2つの値を計算し、$d$がどの範囲に入るかを確認する。"
}
},
{
id: "g4-3",
stage: "第4問",
num: 3,
time: 50,
score: 5,
weakness: "計算精度",
route: ["共通接線の長さ"],
q: "半径9の円Oと半径3の円O'があり、中心間の距離OO'=10である。この2円の共通外接線の長さ(接点間の距離)は？",
a: ["8", "6", "10", "4"],
correct: 0,
tags: ["correct", "calc_error", "formula_mismatch", "calc_error"],
explain: {
aim: "共通外接線の長さの公式(半径の差を使う)を正しく適用できるかを測る問題。",
why: "共通外接線の長さ$L$は、中心間の距離$d$、半径$R,r$を用いて、$L=\\sqrt{d^{2}-(R-r)^{2}}$で求められる。\n\n(この式は、O'から大きい円の半径に垂線を下ろすと、斜辺$d$・一辺$R-r$の直角三角形ができることから、三平方の定理で導ける。)\n\n$d=10$、$R-r=9-3=6$を代入すると、$L=\\sqrt{10^{2}-6^{2}}=\\sqrt{100-36}=\\sqrt{64}=8$。",
mistake: "$R-r$ではなく$R+r$を使ってしまい、別の式(内接線などの式)と混同することがある。",
tip: "共通外接線は『半径の差』、共通内接線は『半径の和』を使う、と覚える。"
}
},
{
id: "g4-4",
stage: "第4問",
num: 4,
time: 60,
score: 5,
weakness: "方針切替",
route: ["2円の位置関係", "共通接線の長さ"],
q: "半径4の円Oと半径1の円O'があり、中心間の距離OO'=5である。まず2円の位置関係を判定し、その上で共通外接線の長さを求めよ。共通外接線の長さは？",
a: ["4", "3", "5", "2"],
correct: 0,
tags: ["correct", "calc_error", "concept_gap", "formula_mismatch"],
explain: {
aim: "2円の位置関係を判定した上で、外接していても共通外接線の公式が同じ形で使えることを理解しているかを測る問題。",
why: "まず位置関係を確認する。$R+r=4+1=5$で、これは中心間の距離$d=5$と等しい。\n\nしたがって2円は$d=R+r$の関係にあり、外接する(1点で接する)。\n\n外接する場合でも、接点を通らない共通外接線は存在し、長さは$L=\\sqrt{d^{2}-(R-r)^{2}}$で求められる。\n\n$d=5$、$R-r=4-1=3$を代入すると、$L=\\sqrt{5^{2}-3^{2}}=\\sqrt{25-9}=\\sqrt{16}=4$。",
mistake: "外接している(1点で接している)と分かった時点で、接点を通らない共通外接線も存在することに気づかず、答えを0や接点での接線と考えてしまうことがある。",
tip: "外接していても、共通外接線(接点を通らない方)は$R+r$ではなく$R-r$を使う公式のまま求められる。"
}
},

/* =========================
第4問(共通点の抽出)
========================= */
{
id: "s5-1",
stage: "第4問",
num: 5,
time: 60,
score: 5,
weakness: "共通点抽出",
route: ["共通点の抽出", "チェバの定理"],
q: "△ABCの辺AB上に点F、辺BC上に点D、辺CA上に点Eがあり、3本の線分AD、BE、CFが1点で交わっている。この状況について、太郎さんは次の3つの例で比の値を調べた。\n例1: $\\dfrac{\\mathrm{AF}}{\\mathrm{FB}}=2$, $\\dfrac{\\mathrm{BD}}{\\mathrm{DC}}=3$, $\\dfrac{\\mathrm{CE}}{\\mathrm{EA}}=\\dfrac{1}{6}$\n例2: $\\dfrac{\\mathrm{AF}}{\\mathrm{FB}}=\\dfrac{1}{2}$, $\\dfrac{\\mathrm{BD}}{\\mathrm{DC}}=4$, $\\dfrac{\\mathrm{CE}}{\\mathrm{EA}}=\\dfrac{1}{2}$\n例3: $\\dfrac{\\mathrm{AF}}{\\mathrm{FB}}=3$, $\\dfrac{\\mathrm{BD}}{\\mathrm{DC}}=\\dfrac{1}{3}$, $\\dfrac{\\mathrm{CE}}{\\mathrm{EA}}=1$\nこの3つの例すべてに共通して成り立つ関係はどれか。",
a: ["(AF/FB)×(BD/DC)×(CE/EA)=1", "(AF/FB)×(BD/DC)=6", "(AF/FB)+(BD/DC)+(CE/EA)=6", "AF/FBとBD/DCの値が等しい"],
correct: 0,
tags: ["correct", "partial_match", "calc_error", "concept_gap"],
explain: {
aim: "複数の具体例に共通する関係を、1例だけで早合点せず全部で確認して見抜けるかを測る問題。",
why: "チェバの定理(三角形の3本の線分が1点で交わるとき、頂点を一周する向きで比を掛けると1になる)より$\\dfrac{\\mathrm{AF}}{\\mathrm{FB}}\\times\\dfrac{\\mathrm{BD}}{\\mathrm{DC}}\\times\\dfrac{\\mathrm{CE}}{\\mathrm{EA}}=1$が常に成り立つ。\n\n例1: $2\\times3\\times\\dfrac{1}{6}=1$\n例2: $\\dfrac{1}{2}\\times4\\times\\dfrac{1}{2}=1$\n例3: $3\\times\\dfrac{1}{3}\\times1=1$\n\nすべて積が1になる。",
mistake: "例1だけを見て『$\\dfrac{\\mathrm{AF}}{\\mathrm{FB}}\\times\\dfrac{\\mathrm{BD}}{\\mathrm{DC}}=2\\times3=6$』という関係に気づき、これを共通の関係だと早合点することがある。例2で確かめると$\\dfrac{1}{2}\\times4=2$となり成り立たない。最初の1例だけで一般化してしまうミス。",
tip: "複数の例から法則を見抜くときは、候補を見つけた時点で必ず残りの例にも当てはめて確認する。1例目で『見つけた!』と思った瞬間が一番危ない。"
}
},
];
