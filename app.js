...
const UNIT_META = {
  ...
  vector: {
    ...
    note: "",
    mission: `
      第1問：成分計算・大きさ・単位ベクトル・ベクトルの分解
      第2問：内積と平行・垂直の判定(混同しやすい2条件の区別)
      第3問：内分点・重心・位置ベクトル・共線条件
      第4問：空間座標と球面(大問チェーン形式、最後は振り返り型)
    `,
    questions: questions_vector,
    routeChoices: ROUTE_CHOICES_VECTOR,
    primaryRoutes: ["成分計算", "内積", "平行条件", "垂直条件", "内分点の公式", "位置ベクトル", "共線条件"]
  },
  ...
  shisuu: {
    ...
    mission: `
      ...
      第2問：相加相乗とtの範囲・指数方程式/不等式(底<1の反転に注意)
      第3問：対数の定義・計算法則・真数条件・対数不等式・常用対数の桁数
      第4問：置き換え方程式の大問チェーン(序盤検算の練習)
    `,
    questions: questions_shisuu,
    routeChoices: ROUTE_CHOICES_SHISUU,
    primaryRoutes: ["指数法則", "置き換え(tの式)", "対数の定義", "対数の計算法則", "真数条件", "tの範囲の確認", "対数不等式"]
  },
  ...
  zahyou: {
    ...
    note: "",
    mission: `
      第1問：直線の方程式と交点・垂直な直線(検算習慣)
      第2問：距離の公式(2点間・点と直線・円と直線・複数の距離の比較)
      第3問：円の方程式と平方完成・接する条件
      第4問：領域の判定と最小値、非有界な領域の最大値(大問チェーン形式、最後は振り返り型)
    `,
    questions: questions_zahyou,
    routeChoices: ROUTE_CHOICES_ZAHYOU,
    primaryRoutes: ["直線の方程式", "連立方程式(交点)", "2点間の距離", "円の方程式", "代表点の代入(領域)", "点と直線の距離", "接する条件(接線)"]
  },
  ...
  sankaku: {
    ...
    note: "",
    mission: `
      第1問：基準角と象限による値の判断・相互関係・変換公式
      第2問：加法定理・倍角公式・半角公式・三角関数の合成
      第3問：三角方程式・周期・グラフの最大最小・二次方程式との融合
      第4問：積分への橋渡し(倍角・半角による式変形、最後は定積分まで通し計算)
    `,
    questions: [...questions_sankaku_1, ...questions_sankaku_2],
    routeChoices: ROUTE_CHOICES_SANKAKU,
    primaryRoutes: ["基準角と象限", "相互関係", "加法定理", "倍角公式", "半角公式", "三角方程式", "合成"]
  },
  ...
  kyokusen: {
    ...
    group: "2bc",
    cardButtonClass: "success",
    description: "距離の比の軌跡 / 放物線・楕円・双曲線 / 媒介変数・極方程式",
    note: "",
    mission: `
      第1問：距離の比の条件から軌跡の方程式を立てて、円の標準形に変形する(模試の実弾チェーン)
    `,
    questions: questions_kyokusen,
    routeChoices: ROUTE_CHOICES_KYOKUSEN,
    primaryRoutes: ["比の条件の立式", "距離の2乗", "式の展開整理", "円の標準形への変形"]
  }
};
...
/* =========================
   Enterキー対応
   クリック/タップ以外に、キーボードのEnterでも
   「開始」「この答えで確定する」「方針を確定」「採点する」「次へ」を押せるようにする。
   今画面に出ている該当ボタンを優先順位順に探してクリックする方式。
========================= */
function isActionable(elm) {
  return !!elm && !elm.disabled && elm.offsetParent !== null;
}

document.addEventListener("keydown", (e) => {
  if (e.key !== "Enter" || e.isComposing) return;

  // 単元切り替えの確認モーダルが開いているときは、背後のボタンを誤爆させない
  const unitModal = el("unitModal");
  if (unitModal && unitModal.style.display !== "none") return;
